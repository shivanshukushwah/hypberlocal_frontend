...
const ShopPage = () => {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-12 space-y-4">
            ...
            <div className="text-center animate-pulse text-emerald-600 font-black uppercase tracking-widest text-xs">Loading shop inventory...</div>
        </div>
    );
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6 sm:gap-8 w-full animate-[fade-in_0.3s_ease-out]">
            ...
            <div className="flex-grow">
                <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-slate-400 hover:text-emerald-600 font-bold mb-4 sm:mb-6 text-xs uppercase tracking-widest transition-colors">
                ...
                {products.length === 0 ? (
                    <div className="glass p-8 sm:p-12 text-center rounded-[2rem] border border-emerald-50 shadow-inner">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
                        {products.map(p => (
                            <div key={p._id} className="glass rounded-[2rem] overflow-hidden hover-scale group relative pb-14 sm:pb-16 flex flex-col border border-transparent hover:border-emerald-100 transition-all">
                                <div className="h-40 sm:h-64 bg-slate-50 relative border-b border-slate-50 shrink-0 overflow-hidden">
                                    {p.imageUrl ? (
                                        ...
                                        <div className="w-full h-full flex flex-col justify-center items-center text-slate-300 font-bold bg-emerald-50/30">
                                    ...
                                    <div className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-white/95 backdrop-blur rounded-full px-2.5 py-1 sm:px-4 sm:py-1.5 text-[9px] sm:text-[10px] font-black text-emerald-700 shadow-sm z-10 uppercase tracking-widest border border-emerald-50">
                                        {p.category}
                                    </div>
                                </div>
                                <div className="p-4 sm:p-6 flex-grow">
                                    <h4 className="text-sm sm:text-xl font-black text-slate-800 line-clamp-1 group-hover:text-emerald-600 transition-colors tracking-tight">{p.name}</h4>
                                    ...
                                    <p className="text-xl sm:text-2xl font-black text-emerald-600 mt-2 sm:mt-4">₹{p.price}</p>
                                </div>

                                {/* Sizes & Add to Cart Action */}
                                <div className="absolute bottom-0 left-0 w-full p-3 sm:p-5 bg-white/80 backdrop-blur border-t border-emerald-50/50 flex items-center justify-between gap-2">
            ...
            <div className="w-full lg:w-96 shrink-0 mt-8 lg:mt-0">
                <div className="glass p-6 sm:p-8 rounded-[2.5rem] sticky top-24 border border-emerald-100 shadow-2xl shadow-emerald-500/10">
                    <h3 className="text-xl sm:text-2xl font-black flex items-center gap-3 mb-6 sm:mb-8 tracking-tight">
                        <ShoppingCart className="text-emerald-600" size={24} sm:size={28} /> Your Bag
                    </h3>
                            ...
                            {cart.items.map((item, idx) => (
                                <div key={idx} className="flex justify-between items-center bg-slate-50/50 p-3 sm:p-4 rounded-2xl border border-slate-100 group">
                                    <div className="min-w-0">
                                        <p className="font-black text-xs sm:text-sm text-slate-800 truncate group-hover:text-emerald-600 transition-colors">{item.product.name}</p>
                                        <p className="text-[10px] sm:text-xs text-slate-500 font-bold uppercase tracking-wider mt-1">{item.size} • Qty: {item.quantity}</p>
                                    </div>
                                    <p className="font-black text-emerald-600 text-xs sm:text-base whitespace-nowrap ml-4">₹{item.price * item.quantity}</p>
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    <div className="border-t border-slate-100 pt-6 mb-6 sm:mb-8">
                        <div className="flex justify-between items-center font-black text-slate-800">
                            ...
                            <span className="text-2xl sm:text-3xl font-black text-emerald-600">₹{cart.shopId === id ? cart.totalAmount : 0}</span>
                        </div>
                    </div>
