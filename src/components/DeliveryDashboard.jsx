                ...
                {myDeliveries.filter(o => o.status !== 'DELIVERED').length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        {myDeliveries.filter(o => o.status !== 'DELIVERED').map(order => (
                            <div key={order._id} className="bg-gradient-to-br from-emerald-50 to-blue-50 border border-emerald-100 p-6 rounded-3xl shadow-sm relative overflow-hidden group">
                                ...
                                <div className="relative z-10">
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <span className="font-extrabold text-emerald-600 bg-emerald-100 px-3 py-1 rounded-lg text-xs tracking-wide">{order.status}</span>
                                    </div>
                                    ...
                                    <div className="flex gap-2">
                                        {order.status === 'PLACED' || order.status === 'READY_FOR_PICKUP' || order.status === 'PREPARING' ? (
                                            <button onClick={() => updateStatus(order._id, 'OUT_FOR_DELIVERY')} className="flex-1 py-3 bg-emerald-600 text-white font-bold rounded-xl text-sm shadow hover:bg-emerald-700 transition-colors">Start Delivery</button>
